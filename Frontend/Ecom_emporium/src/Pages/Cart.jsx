...
const Cart = () => {
  ...
  const fetchCartItems =  () => {
      fetch("http://localhost:8084/cart/products",{
        method:"GET",
  ...
  const handleIncrease = async (cartItemId) => {
    try {
      await axios.put(`http://localhost:8084/cart/increase/${cartItemId}`);
      fetchCartItems(); 
    } catch (error) {
  ...
  const handleDecrease = async (cartItemId) => {
    try {
      await axios.put(`http://localhost:8084/cart/decrease/${cartItemId}`);
      fetchCartItems();
    } catch (error) {
